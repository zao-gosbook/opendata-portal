(function ($) {
  Drupal.behaviors.od_cell_popup_popup = {
    attach: function (context, settings) {
      var config = settings.od_map.world;
      var width = 960,
        height = 500;
      ...
      var wrapper = d3.select(".od-map-wrapper.map-world");
      ...
      function ready(error, map, data) {
        ...
        svg.append("g")
          ...
          .attr("class", function(d) {
            return portalsById[d.id] !== undefined ? 'has-portal' : 'no-portal'
          })
          .style("opacity", 0.8)
          ...
          .on("mouseover", function(d) {
            // Show tooltip
            if (portalsById[d.id] !== undefined) {
              var mouseCoords = d3.mouse(this);
              infoDiv.transition().duration(300)
                .style("opacity", 1);

              infoDiv.html(portalsById[d.id].text + '<span class="second">' + portalsById[d.id].url + '</span>')
          ...
          .on('click', function(d) {
            if (portalsById[d.id] !== undefined) {
              var win = window.open(portalsById[d.id].url);
              win.focus();
            }
          });
